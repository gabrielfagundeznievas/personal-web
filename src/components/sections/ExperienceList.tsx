  ...
  return (
    <div className="space-y-6">
      {experiences.map((job, index) => {
        const translation = t.experiences[index];
            ...
            <div className={`p-8 bg-slate-900/40 relative ${job.highlight ? 'bg-linear-to-br from-slate-900/80 to-emerald-950/20' : ''}`}>
              ...
              <div className="md:flex justify-between items-start mb-4">
                ...
                <div className={`mt-2 md:mt-0 px-3 py-1 rounded text-xs font-mono border ${
                  ...
                  {job.period.replace('ACTUALIDAD', t.experience.current)}
                </div>
              </div>
              ...
              <div className="flex gap-2 flex-wrap">
                {job.tech.map(tech => (
                  <span key={tech} className={`text-[10px] md:text-xs font-bold px-2 py-1 rounded flex items-center gap-1 ${
                    job.highlight
